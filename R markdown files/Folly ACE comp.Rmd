...
#all
all_sites$Location = factor(all_sites$Location, levels=c("Fish Creek", "St. Pierre Creek", "Folly Beach County Park (side B)"), labels=c("Harvested ACE", "Unharvested ACE", "Folly Beach"))

ggplot(na.omit(all_sites), aes(x = Growth.bands, y = Length.mm, group = Location)) + geom_point(aes(colour = Location), alpha = 0.3) + geom_smooth(method = "nls", formula = y ~ Linf * (1 - exp(-K * (x - t0))), method.args = list(start=c(Linf = 108.54, K = 0.29, t0 = -0.65)), se=F, aes(col = Location)) + scale_y_continuous(name = "Total length (mm)", limits = c(0, 120)) + xlab("Age (growth bands)") + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+theme(axis.title = element_text(size = 15), strip.text = element_text(size=15), axis.text.x = element_text(size=13), axis.text.y = element_text(size = 13)) + theme(legend.text=element_text(size=8))
