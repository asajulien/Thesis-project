...
elev_x8 = update(elev_x5, .~. - MHW)

anova(full_x, elev_x, test='LRT')
AIC(full_x, elev_x)

anova(full_x, elev_x2, elev_x4, elev_x5, elev_x6, elev_x7, elev_x8, test='LRT')
AIC(full_x, elev_x2, elev_x3, elev_x4, elev_x5, elev_x6, elev_x7, elev_x8) #model 5 is the best
...
plot(c(0,2),c(0,1),type = 'n', axes = F,xlab = '', ylab = '', main = 'Salinity (ppt)')
text(x=1.5, y = seq(0,1,l=4), labels = seq(23,8,l=4))
...
##Test model
```{r}
follytest = read.csv("C:\\Users\\juliena\\Desktop\\Data\\Elevation\\CSV files\\FBCP_elevation_survey.csv")

follytest$Mussel.presence = ifelse(follytest$Mussel.presence == 'y', 1, 0)
follytest$salinity = rep(25) #JUST A GUESS, get some real data

follytest = follytest[,c(5, 15, 44)]

fit_folly = predict(elev_x5, newdata=follytest,
                  type='response')

g_folly = roc(Mussel.presence ~ fit_folly, data = follytest)
plot(g_folly)
auc(g_folly)#Not great at all. I think it's too small of a dataset though, with not enough of a range in elevation.
```
