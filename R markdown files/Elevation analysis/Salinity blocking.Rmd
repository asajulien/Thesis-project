...
Sal_dist = read.csv("C:\\users\\juliena\\desktop\\data\\elevation\\salinity transect\\csv files\\Salinity_distance.csv")
...
alpha = coef(fit)
plot(Salinity ~ Distance.m, data = Sal_dist, xlab = "Distance downstream (m)", ylab = "Salinity (ppt)", cex.lab = 1, pch =17, las = 1)
curve(alpha[1]/(1 + exp(-(x - alpha[2])/alpha[3])), add = T, col = "black")  # Fitted model
curve(24.525/(1 + exp(-(x - 13106.12)/3953.79)), add = T, col = "red")

xxx = Sal_dist$xxx
fitxxx = 24.525/(1 + exp(-(xxx - 13106.12)/3953.79))

points(xxx, fitxxx, col='black', pch = 1)
