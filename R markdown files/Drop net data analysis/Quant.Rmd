...
elev_mod4 = update(elev_mod2, . ~. + Block + Elevation:Block)

anova(full_mod, main_mod, elev_mod, test='LRT')
anova(elev_mod, elev_mod2, elev_mod3, elev_mod4, test='LRT')
#add salinity back in - plot both
plot(elev_mod2)

new_data = data.frame(Elevation = seq(0, .8, .05))
plot(Mussel.presence ~ Elevation, data=ashgrid)
...
lines(new_data$Elevation, predict(elev_mod4, newdata=new_data, type='response'),
      col='green') #This isn't right and I have no idea how to do this...
