...
p_seasrich = ggplot(rich_season, aes(Treatment, Richness), position="dodge") + facet_grid(~Season)
p_seasrich + geom_bar(stat="identity") + geom_errorbar(aes(ymax = Richness + SE.rich, ymin = Richness - SE.rich), width = 0.25) + facet_grid(~Season) + labs(x = "Treatment", y = "Simpson diversity index") + theme(axis.title = element_text(size = 20,face = "bold"), strip.text = element_text(size=18,face = "bold"), axis.text.x = element_text(size=13,face = "bold"), axis.text.y = element_text(size = 13, face = "bold"))
```
...
ggplot(spp_df, aes(Month, Lx, group = 1)) + geom_point() + geom_line() + geom_errorbar(aes(ymax = Lx + se.lx, ymin = Lx - se.lx), width=0.25)

#C. sapidus size
csap_df = legare_net %>%
  filter(Species == "Callinectes sapidus")
csap_df$Month = c(rep("July 2016", 4), rep("August 2016", 17), rep("September", 28), rep("October", 15), rep("November", 14), rep("December", 6), rep("January", 1), rep("February", 4), rep("March", 6), rep("April", 4), rep("May", 3), rep("June 2017", 1), rep("July 2017", 1), rep("August 2017", 1))
csap_df$Month = factor(csap_df$Month, levels = c("July 2016", "August 2016", "September", "October", "November", "December", "January", "February", "March", "April", "May", "June 2017", "July 2017", "August 2017"))

ggplot(data = csap_df, aes(Total.Length..mm.)) + geom_histogram(binwidth = 5) + facet_grid(.~Month) + labs(x = "Length (mm)", y = "Frequency (number)")
