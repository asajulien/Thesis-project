...
#ggplot, by month
fbcp_freq$Month.code = factor(fbcp_freq$Month.code, levels = c("August","September","October","November","December", "January", "February", "March", "April", "May", "June")) #reordering the months
...
#percent hists - there must be a way to do this with geom_hist and faceting... the (..count../sum(..count..) thing can be a mess...)
pg13 = ggplot_build(sizeage.plot)
cp3 = data.frame(pg13$data)
cp4 = cp3 %>%
  group_by(PANEL) %>%
  mutate(count/sum(count)) #WORKS, need barplot

cp4$percent = cp4$`count/sum(count)`
cp4$Bin = as.factor(cp4$x)

lj23 = ggplot(cp4, aes(x = Bin, y = percent))
lj23 + facet_grid(~PANEL) + geom_bar(stat = "identity") #BASIC IDEA, NEEDS FORMATTING


sizeage.plot <- ggplot(data = fbcp_freq, aes(x = Length.mm)) + geom_histogram(aes(y=(..count..)/sum(..count..)), binwidth = 5) + facet_grid(~Month.code) + theme_bw() + labs(x = "Length (mm)", y="Frequency") 
...
#plots
mn.lengths.fbcp$Month<-factor(mn.lengths.fbcp$Month, levels = c("August","September","October","November","December", "January", "February", "March", "April", "May")) #Ordering the months
