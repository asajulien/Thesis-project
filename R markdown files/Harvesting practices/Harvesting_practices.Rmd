...
p_mass + geom_bar(stat = "identity", width = 0.7) + geom_errorbar(aes(ymin = Wt - se.wt, ymax = Wt + se.wt), width = 0.1) + labs(x = "", y = "Weight (g)") + annotate("text", x = "Full", y = 62, label = "*", size = 10)
...
dens_glm = aov(Density ~ Treatment, data = stemdensity) #no effect of block
summary(dens_glm)
plot(dens_glm)
...
ggplot(stem_mn, aes(Treatment, mn_dens)) + geom_bar(stat = "identity", width = 0.7) + geom_errorbar(aes(ymin = mn_dens - se_dens, ymax = mn_dens + se_dens), width = 0.1) + labs (x = "", y = "Stem density") #Conclusion: While it appears that there is a difference in stem density based on treatment, only the block effect is significant, according to the quasipoisson regression
...
ggplot(stems, aes(y = Length, x = Treatment)) + geom_boxplot() + labs(x = "", y = "Length (mm)")
...
ggplot(flower_mn, aes(Treatment, mn_flower)) + geom_bar(stat = "identity", width = 0.7) + geom_errorbar(aes(ymin = mn_flower - se_flower, ymax = mn_flower + se_flower), width = 0.1) + ylab("Percent flowering") + xlab("") + annotate("text", x = "Full", y = 9.5, label = "*", size = 10)
...
yoty = recruits %>%
  ...
  summarise(length(Bands[Bands == 0])) #nothing with 1 growth band
...
ggplot(yoty, aes(Treatment, Recruits)) + geom_bar(stat = "identity", width = 0.5) + scale_y_continuous("Number of recuits", breaks = seq(0, 12, 4)) + xlab("")
